...
const expected={
  ...
  repositoryModel:["buildRepositoryModelPacket","emitRepositoryModelBundle","EXTRACTOR_VERSIONS","INTERPRETATION_PROFILE_HASH","INTERPRETATION_PROFILE_ID","INTERPRETATION_PROFILE_VERSION","interpretationProfile","interpretRepository","observeRepositoryModel","REPOSITORY_MODEL_PACKET_TYPE","REPOSITORY_MODEL_PACKET_VERSION","REPOSITORY_MODEL_PRODUCER_NAME","validateRepositoryModelPacket"],
};
...
assert.strictEqual(typeof repositoryModel.interpretRepository,"function");
assert.match(repositoryModel.interpretationProfile().hash,/^sha256:[0-9a-f]{64}$/);
