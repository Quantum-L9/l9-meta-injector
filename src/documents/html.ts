...
export const HTML_DECODER_ID = "l9.html-decoder";
export const HTML_DECODER_VERSION = "1.0.0";
...
const BLOCK_ELEMENTS: Readonly<Record<string, BlockKind>> = {
  h1: "heading",
  h2: "heading",
  h3: "heading",
  h4: "heading",
...
function parseHtmlAttributes(raw: string): Record<string, string> {
  ...
  while (match !== null) {
    ...
    attributes[(match[1] as string).toLowerCase()] = decodeXmlText(value);
...
export const htmlDecoder: DocumentDecoder = {
  id: HTML_DECODER_ID,
  version: HTML_DECODER_VERSION,
  format: "html",
  ...
  decode(input: DecodeInput): DecodeOutcome {
    ...
    const documentId = normalizedDocumentId({
      ...
      decoderId: HTML_DECODER_ID,
      decoderVersion: HTML_DECODER_VERSION,
    });
    ...
    let opaqueDepth = 0;
    let nodeIndex = 0;
    let pending: { kind: BlockKind; name: string; path: string; parts: string[] } | null = null;
    let inTitle = false;
    ...
    const flushPending = (): void => {
      ...
      if (text.trim().length > 0) {
        ...
        const kind: BlockKind = pending.kind === "heading" && pending.name === "h1" && !sawTitleBlock
          ? "title"
          : pending.kind;
        if (kind === "title") sawTitleBlock = true;
    ...
    for (const token of tokenizeHtml(read.text)) {
      ...
      if (token.kind === "open") {
        if (OPAQUE.has(token.name)) {
          if (token.name === "script" || token.name === "noscript") sawScript = true;
          opaqueDepth += 1;
      ...
      if (token.kind === "text") {
        ...
        const text = decodeXmlText(token.text);
        if (inTitle) titleParts.push(text);
        if (anchorHref !== null) anchorText.push(text);
        if (pending !== null) pending.parts.push(text);
        continue;
      }
      ...
      if (pending !== null && pending.name === token.name) flushPending();
      if (token.name === "tr") {
    ...
    return {
      ...
      document: buildNormalizedDocument({
        decoder: { id: HTML_DECODER_ID, version: HTML_DECODER_VERSION, format: "html" },
        decodeInput: input,
        documentId,
        metadata,
        builder,
