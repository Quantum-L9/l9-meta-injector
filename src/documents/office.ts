...
import { OoxmlContainer, OoxmlError, noteExternalRelationships, openOoxml } from "./ooxml";
import { XmlError, parseXml } from "./xml";

export const DOCX_DECODER_ID = "l9.docx-decoder";
export const DOCX_DECODER_VERSION = "1.0.0";
export const PPTX_DECODER_ID = "l9.pptx-decoder";
export const PPTX_DECODER_VERSION = "1.0.0";
export const XLSX_DECODER_ID = "l9.xlsx-decoder";
export const XLSX_DECODER_VERSION = "1.0.0";

/** Turn an OOXML failure into the decode outcome it deserves. */
function ooxmlFailure(error: unknown): DecodeOutcome {
...
function coreProperties(container: OoxmlContainer, maxDepth: number): Record<string, string> {
  ...
  try {
    parseXml(xml, maxDepth, (event) => {
      ...
      if (event.type === "close") current = null;
    });
  } catch (error) {
 ...
 */
function docxParagraphKind(style: string | null, listLevel: number | null): BlockKind {
  if (style !== null) {
    const normalized = style.toLowerCase();
    if (normalized === "title") return "title";
    if (normalized.startsWith("heading") || /^h[1-9]$/.test(normalized)) return "heading";
  }
  return listLevel !== null ? "list_item" : "paragraph";
}

export const docxDecoder: DocumentDecoder = {
  id: DOCX_DECODER_ID,
  version: DOCX_DECODER_VERSION,
  format: "docx",
  ...
  decode(input: DecodeInput): DecodeOutcome {
    ...
    let container: OoxmlContainer;
    try {
      container = openOoxml(input.absolutePath, input.budget);
    } catch (error) {
    ...
    const documentId = normalizedDocumentId({
      ...
      decoderId: DOCX_DECODER_ID,
      decoderVersion: DOCX_DECODER_VERSION,
    });
    ...
    try {
      parseXml(xml, input.budget.maxDepth, (event) => {
        if (builder.isFull) return false;
        if (event.type === "open") {
          switch (event.name) {
            case "del":
            case "delText":
            ...
            case "ilvl":
              ...
              if (!Number.isFinite(listLevel)) listLevel = null;
              return;
            case "tbl":
              inTable = true;
              tableRows = [];
    ...
    for (const diagnostic of container.diagnostics) builder.note(diagnostic);
    for (const diagnostic of noteExternalRelationships(container, input.budget)) builder.note(diagnostic);

    return {
      decoded: true,
      document: buildNormalizedDocument({
        decoder: { id: DOCX_DECODER_ID, version: DOCX_DECODER_VERSION, format: "docx" },
        decodeInput: input,
        documentId,
...
function slideOrder(name: string): number {
  ...
  return match === null ? Number.MAX_SAFE_INTEGER : Number.parseInt(match[1] as string, 10);
}

export const pptxDecoder: DocumentDecoder = {
  id: PPTX_DECODER_ID,
  version: PPTX_DECODER_VERSION,
  format: "pptx",
  ...
  decode(input: DecodeInput): DecodeOutcome {
    ...
    const documentId = normalizedDocumentId({
      contentHash: input.contentHash,
      decoderId: PPTX_DECODER_ID,
      decoderVersion: PPTX_DECODER_VERSION,
    });
    const builder = new BlockBuilder(documentId, input.budget);
    ...
    const readShapes = (part: string, slideNumber: number, noteKind: BlockKind | null): void => {
      const xml = container.readPart(part);
      if (xml === null) return;
      let shapeIndex = 0;
      let runs: string[] = [];
        ...
        parseXml(xml, input.budget.maxDepth, (event) => {
          ...
          if (event.type === "open") {
            switch (event.name) {
              case "ph":
                ...
                if (["title", "ctrTitle"].includes(event.attributes.type ?? "")) isTitle = true;
                return;
              case "tbl":
                inTable = true;
                tableRows = [];
          ...
          switch (event.name) {
            ...
            case "graphicFrame": {
              ...
              shapeIndex += 1;
              const kind: BlockKind = noteKind ?? (isTitle ? "title" : "paragraph");
              isTitle = false;
    ...
    for (const slide of slides) {
      ...
      if (container.entries.has(notes)) readShapes(notes, slideNumber, "note");
    ...
    return {
      decoded: true,
      document: buildNormalizedDocument({
        decoder: { id: PPTX_DECODER_ID, version: PPTX_DECODER_VERSION, format: "pptx" },
        decodeInput: input,
        documentId,
        metadata: coreProperties(container, input.budget.maxDepth),
        builder,
...
function readSharedStrings(container: OoxmlContainer, maxDepth: number): string[] {
...
function readSheets(container: OoxmlContainer, maxDepth: number): { name: string; part: string }[] {
  const workbook = container.readPart("xl/workbook.xml");
  const names: string[] = [];
  if (workbook !== null) {
    try {
      parseXml(workbook, maxDepth, (event) => {
...
export const xlsxDecoder: DocumentDecoder = {
  id: XLSX_DECODER_ID,
  version: XLSX_DECODER_VERSION,
  format: "xlsx",
  ...
  decode(input: DecodeInput): DecodeOutcome {
    ...
    const documentId = normalizedDocumentId({
      contentHash: input.contentHash,
      decoderId: XLSX_DECODER_ID,
      decoderVersion: XLSX_DECODER_VERSION,
    });
    const builder = new BlockBuilder(documentId, input.budget);
    ...
    for (const sheet of sheets) {
      ...
      try {
        parseXml(xml, input.budget.maxDepth, (event) => {
          ...
          if (event.type === "text") {
            if (inFormula) formula.push(event.text);
            else if (inValue || inInline) value.push(event.text);
            return;
          }
          switch (event.name) {
            case "v":
            ...
            case "c": {
              ...
              if (rendered.trim().length > 0 && reference.length > 0) {
                ...
                if (rowMatch !== null) {
                  const rowNumber = Number.parseInt(rowMatch[1] as string, 10);
    ...
    return {
      decoded: true,
      document: buildNormalizedDocument({
        decoder: { id: XLSX_DECODER_ID, version: XLSX_DECODER_VERSION, format: "xlsx" },
        decodeInput: input,
        documentId,
        metadata: coreProperties(container, input.budget.maxDepth),
        builder,
