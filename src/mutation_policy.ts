...
export const INLINE_MANAGED_ARTIFACT_TYPES = new Set<ArtifactType>([
  "prompt",
  "skill",
  "playbook",
  "kernel",
...
const CENTRAL_MANIFEST_ARTIFACT_TYPES = new Set<ArtifactType>([
  "source",
  "test",
  "script",
]);

export type CarrierInjectionStrategy =
  | "yaml-frontmatter"
  | "line-comment"
  | "block-comment"
  | "sidecar"
...
function normalizeRelativePath(value: string): string {
  if (value.includes("\u0000")) throw new Error("carrier path contains NUL");
  if (value.includes("\\")) throw new Error(`carrier path must use POSIX separators: ${value}`);
  if (value.startsWith("/") || value.startsWith("./")) {
    throw new Error(`carrier path must be canonical repository-relative POSIX: ${value}`);
  }
  if (value.includes("//")) throw new Error(`carrier path contains an empty segment: ${value}`);
  const segments = value.split("/");
  if (segments.some((segment) => !segment || segment === "." || segment === "..")) {
    throw new Error(`carrier path contains a non-canonical segment: ${value}`);
  }
  const normalized = segments.join("/");
  if (!normalized) throw new Error("carrier path must identify a repository entry");
  return normalized;
}
...
function posixExtname(value: string): string {
...
function startsWithAny(relativePath: string, prefixes: readonly string[]): string | undefined {
  return prefixes.find((prefix) => relativePath.startsWith(prefix));
}

function decision(pathName: string, carrier: MetadataCarrier, reason: string, authorityRule?: string): CarrierDecision {
...
export function resolveCarrierDecision(
  ...
  if (inlinePattern && inlineEligible) {
    return decision(
      relativePath,
      "inline_managed",
      mode === "skills" && basename === "skill.md"
        ? "explicitly authorized managed skill entrypoint"
        : "explicitly authorized L9-managed prose artifact",
  ...
  if (inlinePattern && !inlineEligible) {
    return decision(
      relativePath,
      "central_manifest",
      `inline_allow cannot override ineligible artifact type '${subject.artifactType}'`,
      `inline_allow:${inlinePattern}`,
    );
  }
  ...
  return decision(
    relativePath,
    "central_manifest",
    authority.default_carrier === "inline_managed"
...
export function resolveCarrierDecisions(input: CarrierPolicyInput): CarrierDecision[] {
  const seen = new Set<string>();
  const decisions = input.subjects.map((subject) => {
    const result = resolveCarrierDecision(subject, input.authority, input.mode);
    if (seen.has(result.path)) throw new Error(`duplicate carrier subject path: ${result.path}`);
...
export function assertCarrierDecisionCoverage(
  subjects: readonly CarrierSubject[],
  decisions: readonly CarrierDecision[],
): void {
  ...
  for (const item of decisions) {
    ...
    if (actual.has(normalized)) throw new Error(`duplicate carrier decision path: ${normalized}`);
    actual.add(normalized);
    if (!CARRIER_PRECEDENCE.includes(item.carrier)) {
