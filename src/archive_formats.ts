...
export const EXPANDABLE_ARCHIVE_EXTENSIONS: ReadonlySet<string> = new Set([".zip"]);
 ...
 */
export const KNOWN_UNEXPANDABLE_ARCHIVE_EXTENSIONS: ReadonlySet<string> = new Set([
...
export const ARCHIVE_EXTENSIONS: ReadonlySet<string> = new Set([
  ...
  ...KNOWN_UNEXPANDABLE_ARCHIVE_EXTENSIONS,
]);

/** Lower-cased extension of a path or member name, as `path.extname` reports it. */
export function archiveExtensionOf(name: string): string {
  return path.extname(name).toLowerCase();
}

export function isExpandableArchivePath(name: string): boolean {
  return EXPANDABLE_ARCHIVE_EXTENSIONS.has(archiveExtensionOf(name));
}

export function isKnownUnexpandableArchivePath(name: string): boolean {
  return KNOWN_UNEXPANDABLE_ARCHIVE_EXTENSIONS.has(archiveExtensionOf(name));
}

export function isArchivePath(name: string): boolean {
  return ARCHIVE_EXTENSIONS.has(archiveExtensionOf(name));
}
...
export const ARCHIVE_SIGNATURE_PROBE_BYTES = 512;
 ...
 */
export function sniffArchiveSignature(prefix: Buffer): ArchiveSignature | null {
  const at = (offset: number, bytes: number[]): boolean =>
    prefix.length >= offset + bytes.length
  ...
  if (at(0, [0x42, 0x5a, 0x68]) && prefix.length > 3 && prefix[3] >= 0x31 && prefix[3] <= 0x39) return "bzip2";
  ...
  if (at(USTAR_OFFSET, [0x75, 0x73, 0x74, 0x61, 0x72]) && prefix.length > USTAR_OFFSET + 5
    && (prefix[USTAR_OFFSET + 5] === 0x00 || prefix[USTAR_OFFSET + 5] === 0x20)) return "tar";
  return null;
}
 ...
 */
export const ZIP_CONTAINER_DOCUMENT_EXTENSIONS: ReadonlySet<string> = new Set([
 ...
 */
export function signatureContradictsName(name: string, signature: ArchiveSignature): boolean {
  const extension = archiveExtensionOf(name);
  if (ARCHIVE_EXTENSIONS.has(extension)) return false;
  if (signature === "zip" && ZIP_CONTAINER_DOCUMENT_EXTENSIONS.has(extension)) return false;
  return true;
}
