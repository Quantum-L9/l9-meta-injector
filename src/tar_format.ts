...
export function encodeTarEntry(spec: TarHeaderFields): Buffer {
...
export function encodeTarArchive(entries: TarHeaderFields[]): Buffer {
  return Buffer.concat([...entries.map(encodeTarEntry), Buffer.alloc(BLOCK * 2)]);
}

export function parseOctalField(raw: Buffer): number {
...
export function tarChecksum(header: Buffer): number {
  let sum = 0;
