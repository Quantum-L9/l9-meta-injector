...
`src/pipeline.ts#runPipelineAsync` is the stable orchestration contract. It accepts `PipelineConfig` and returns `PipelineResult`, including verification, coverage, placement plans, MetaV3 records, and metrics. Optional `localFiles` (ADR-016) materializes `.zip` archives beside themselves before scan, for disposable local trees that the operator intends to modify; default mode never extracts. Since ADR-045 that materialization is transactional — members land in a same-directory candidate, the v2 ownership marker is written only after every member is verified, and the candidate is atomically swapped into place — and both archive paths admit through one `ArchiveExecutionContext` (`src/archive_execution.ts`). For read-only observation of an arbitrary file, folder, drive tree or ZIP, use `src/local_source_model.ts#observeLocalSourceModel` (ADR-036), which never mutates the source.
