...
| 16 | Injection overwrites existing headers, including legacy `L9_META`-family blocks | [ADR-016](decisions/016-overwrite-existing-headers-on-injection.md) | An injected file must carry exactly one authoritative header; stale headers are updated governed metadata, never preserved beneath a fresh one. |
