...
def _manifest_gate(move_map):
    ...
    for r in rows:
        if (
            not r.get("source_path")
            or not r.get("content_hash")
            or not r.get("output_path")
        ):
...
def phase_b(source, output, threshold):
    ...
    with open(os.path.join(man_dir, "conflicts.md"), "w") as fh:
        ...
        fh.write("# Conflicts\n\n## Duplicates\n")
        for r in dupes:
            fh.write(f"- {r['source_path']} ({r['content_hash'][:12]})\n")
        if not dupes:
            fh.write("- none\n")
...
def run(a):
    ...
    if a.phase == "B":
        ...
        if not a.manifest:
            sys.exit("[C] FAIL: --from-manifest required")
