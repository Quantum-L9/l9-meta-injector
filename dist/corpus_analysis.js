...
exports.duplicateRepresentative = duplicateRepresentative;
exports.compareDuplicateClusters = compareDuplicateClusters;
exports.clusterExactDuplicates = clusterExactDuplicates;
 ...
 */
function prefixLength(setSize, threshold) {
    if (setSize === 0)
        return 0;
...
function nearDuplicateCandidates(documents, threshold) {
    ...
    const ordered = documents
        ...
        .sort((left, right) => left.document.shingles.size - right.document.shingles.size
        || (0, ordering_1.compareCodePoints)(left.document.artifactId, right.document.artifactId));
    ...
    for (let position = 0; position < ordered.length; position++) {
        ...
        for (const shingle of prefix) {
            for (const other of postings.get(shingle) ?? []) {
                ...
                if (partner.shingles.size < roundScore(threshold) * size)
                    continue;
                if (pairIneligible(current, partner))
                    continue;
 ...
 * the one to keep.
 */
function duplicateRepresentative(members) {
    return [...members].sort((left, right) => left.sourcePath.length - right.sourcePath.length
...
function compareDuplicateClusters(left, right) {
 ...
 * disks and archives.
 */
function clusterExactDuplicates(artifacts) {
    ...
    for (const [hash, group] of byHash) {
        if (group.length < 2)
            continue;
        const members = [...group].sort((left, right) => (0, ordering_1.compareCodePoints)(left.sourcePath, right.sourcePath));
        ...
        clusters.push({
            ...
            artifact_ids: members.map((member) => member.artifactId).sort(ordering_1.compareCodePoints),
