...
function splitLines(raw, start = 0) {
    const out = [];
    let offset = start;
    let number = 1;
...
function renderSafeScalar(value) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new Error("managed frontmatter values must be finite numbers");
        return String(value);
    }
...
function looksLikeDuplicateBlock(raw, start) {
    ...
    for (let j = i + 1; j < Math.min(lines.length, i + 201); j++) {
        if (lines[j].text === "---")
            return true;
        if (lines[j].text === "..." || lines[j].text.startsWith("# "))
            continue;
    }
    return false;
}
function inspectFrontMatterDocument(raw) {
    ...
    const emptyMeta = Object.create(null);
    if (first.text !== "---") {
    ...
    while (i < headerLines.length) {
        ...
        if (/^\s/.test(line.text))
            return issue(raw, "FRONTMATTER_COMPLEX_YAML", "unexpected indentation outside a supported scalar sequence", { line: line.number });
        if (/^(?:%|\.\.\.|\?|!|&|\*|\{|\}|<<:)/.test(line.text))
            return issue(raw, "FRONTMATTER_COMPLEX_YAML", "unsupported YAML directive, tag, anchor, alias, explicit key, or map", { line: line.number });
        const match = /^([A-Za-z_][A-Za-z0-9_-]*):(.*)$/.exec(line.text);
        if (!match)
            return issue(raw, "FRONTMATTER_INVALID_KEY", "frontmatter must contain simple top-level key/value fields", { line: line.number });
        const key = match[1];
        if (seen.has(key))
    ...
    return {
        ...
        hadFrontMatter: true,
        bom,
        newline: ([...headerNewlines].find((value) => value === "\n" || value === "\r\n") ?? newline),
...
function failureFromInspection(raw, inspection, managedKeys) {
    return { ...inspection, content: raw, changed: false, managedKeys };
}
function patchManagedFrontMatter(raw, managed) {
    ...
    if (!inspection.safe)
        return failureFromInspection(raw, inspection, managedKeys);
    ...
    if (!inspection.hadFrontMatter) {
        ...
        if (!next.safe)
            return failureFromInspection(raw, next, managedKeys);
    ...
    for (const key of managedKeys) {
        ...
        if (field.kind === "scalar") {
            ...
            const sequence = managed[key].map((item) => `${indent}- ${renderSafeScalar(item)}${newline}`).join("");
            edits.push({
                start: field.start,
                end: field.end,
...
//# sourceMappingURL=frontmatter_patch.js.map