...
exports.stripLeadingLegacyMetaBlock = stripLeadingLegacyMetaBlock;
...
function stripLeadingLegacyMetaBlock(body) {
    const lines = body.split("\n");
    const head = lines[0]?.startsWith("#!") ? 1 : 0; // keep a shebang on line 1
    for (;;) {
        let i = head;
        while (i < lines.length && lines[i].trim() === "")
            i++; // skip leading blank lines
        // An opener bears the token but is not itself the closing sentinel.
        if (i >= lines.length || !LEGACY_META_OPEN.test(lines[i]) || LEGACY_META_CLOSE.test(lines[i]))
            break;
        let j = i + 1;
        while (j < lines.length && !LEGACY_META_CLOSE.test(lines[j]))
            j++;
        if (j >= lines.length)
            break; // no closing sentinel → not a delimited block; leave verbatim
        let end = j + 1;
        if (end < lines.length && lines[end].trim() === "")
            end++; // consume one trailing blank
