...
exports.corpusIntelligenceInput = corpusIntelligenceInput;
const corpus_intelligence_1 = require("./corpus_intelligence");
 ...
 */
const LINE_BEARING_FORMATS = new Set([
    "text",
    "markdown",
    "csv",
    "html",
...
const UPSTREAM_REASONING_TYPES = new Set([
    "NONE",
...
const CONFIDENCE_CLASSES = new Set(["weak", "moderate", "strong"]);
 ...
 */
function identityTranslation(result) {
    ...
    for (const artifact of result.snapshot.artifacts) {
        ...
        if (resolved !== undefined)
            translated.set(artifact.virtual_source_id, resolved);
    }
    return translated;
}
function translate(identity, mapping) {
 ...
 * it.
 */
function lowerLocator(locator, documentFormat, blockKind, context) {
    switch (locator.kind) {
        case "line_span": {
            if (!LINE_BEARING_FORMATS.has(documentFormat)) {
                throw new corpus_intelligence_1.CorpusIntelligenceError(`${context}: a ${documentFormat} document carries a line locator; that format ` +
        ...
        case "notebook_cell": {
            ...
            if (locator.line_start !== undefined)
                located.start_line = locator.line_start;
            if (locator.line_end !== undefined)
                located.end_line = locator.line_end;
            return located;
        }
        case "pdf_page_block":
        ...
        case "csv_row": {
            const located = { kind: "csv", row: locator.row_number };
            if (locator.column !== undefined && locator.column !== "")
                located.column = locator.column;
            return located;
        }
        case "html_node":
            return {
        ...
        default: {
            const unreachable = locator;
            throw new corpus_intelligence_1.CorpusIntelligenceError(`${context}: unknown locator kind ${JSON.stringify(unreachable)}`);
        }
    }
}
function workSignal(record) {
    ...
    return {
        ...
        locator: lowerLocator(record.structured_locator, record.format, record.block_kind, context),
...
// ───────────────────────────── payload domains ─────────────────────────────
function duplicateRelations(result, identities) {
    return result.candidates.relations
        ...
        .sort((a, b) => (0, ordering_1.compareCodePoints)(a.relation_id, b.relation_id));
}
function pairRelations(result, identities, profile) {
    const semantic = result.semantic;
    if (semantic === null)
        return [];
    const classification = new Map(semantic.relations.classifications.map((entry) => [entry.pair_id, entry]));
    return semantic.relations.pairs
        .map((pair) => {
        const scores = pair.signals
            .map((signal) => ({
            method: signal.method,
            score: (0, repository_model_1.canonicalFloat)(Math.max(0, Math.min(1, signal.score))),
        }))
            .sort((a, b) => (0, ordering_1.compareCodePoints)(a.method, b.method));
        return {
...
function candidateClusters(entries, candidateType, identities, profile) {
    return entries
        .map((entry) => ({
        candidate_id: entry.candidate_id,
        candidate_type: candidateType,
        member_artifact_ids: sortedUnique(entry.member_artifact_ids.map((member) => translate(member, identities))),
        ...
        .sort((a, b) => (0, ordering_1.compareCodePoints)(a.candidate_id, b.candidate_id));
}
/**
 ...
 */
function lexicalProjectClusters(result, identities) {
    ...
    return result.candidates.project_candidates
        ...
        member_artifact_ids: sortedUnique(candidate.member_ids.map((member) => translate(member, identities))),
...
function readinessEvidence(result, subjectByProjectKey) {
    const profile = result.readiness.profile;
    const subject = (projectKey, bodyId) => {
        const resolved = subjectByProjectKey.get(projectKey);
        if (resolved === undefined) {
            throw new corpus_intelligence_1.CorpusIntelligenceError(`readiness body ${bodyId} is about project key ${projectKey}, which this run ` +
    ...
    return result.readiness.bodies_of_work
        ...
        const counts = Object.fromEntries(READINESS_FIELDS.map(([group, sourceKey, field]) => {
            const value = body.metrics[group]?.[sourceKey];
        ...
        .sort((a, b) => (0, ordering_1.compareCodePoints)(a.readiness_id, b.readiness_id));
}
function reasoningRequests(result, identities) {
    const semantic = result.semantic;
    if (semantic === null)
        return { requests: [], packRefs: [] };
    const packByCandidate = new Map(semantic.evidencePacks.map((pack) => [pack.reasoning_candidate_id, pack.evidence_pack_id]));
    const requests = semantic.reasoningCandidates
        .map((row) => {
        if (!UPSTREAM_REASONING_TYPES.has(row.reasoning_type)) {
            throw new corpus_intelligence_1.CorpusIntelligenceError(`reasoning candidate ${row.reasoning_candidate_id} requests ` +
        ...
        return {
            ...
            recommended_reasoning_type: row.reasoning_type,
            reason: row.reason,
            member_artifact_ids: sortedUnique(row.member_artifact_ids.map((member) => translate(member, identities))),
            evidence_pack_ref: packByCandidate.get(row.reasoning_candidate_id) ?? null,
        };
    })
        .sort((a, b) => (0, ordering_1.compareCodePoints)(a.reasoning_candidate_id, b.reasoning_candidate_id));
...
function roots(result) {
    ...
    for (const root of result.snapshot.roots) {
        ...
        if (root.observation_status !== "observed")
            continue;
        const carried = packets.get(root.root_id);
        if (carried === undefined) {
            throw new corpus_intelligence_1.CorpusIntelligenceError(`root ${root.root_id} observed successfully but produced no repository model packet`);
        }
        const packet = carried.packet;
        if (root.rmp_packet_id && root.rmp_packet_id !== packet.packet_id) {
            throw new corpus_intelligence_1.CorpusIntelligenceError(`root ${root.root_id} snapshot names packet ${root.rmp_packet_id}, ` +
                `but its bundle carries ${packet.packet_id}`);
        }
        if (root.rmp_semantic_hash && root.rmp_semantic_hash !== packet.semantic_hash) {
            throw new corpus_intelligence_1.CorpusIntelligenceError(`root ${root.root_id} snapshot names semantic hash ${root.rmp_semantic_hash}, ` +
                `but its bundle carries ${packet.semantic_hash}`);
        }
        observed.push({
            rootId: root.root_id,
            identityClass: root.root_identity_class,
    ...
    if (observed.length === 0) {
        throw new corpus_intelligence_1.CorpusIntelligenceError("no root in this run observed successfully");
    }
    return observed;
}
/**
 ...
 */
function corpusIntelligenceInput(result, options) {
    ...
    const subjectByProjectKey = new Map(result.candidates.project_candidates.map((candidate) => [
    ...
    const payload = {
        document_work_signals: result.documentWorkSignals.records
            .map(workSignal)
            .sort((a, b) => (0, ordering_1.compareCodePoints)(a.signal_id, b.signal_id)),
        ...
        topic_candidates: semantic === null
            ? []
        ...
        consolidation_candidates: semantic === null
            ? []
            : candidateClusters(semantic.consolidations.candidates, "CONSOLIDATION_CANDIDATE", identities, profile),
...
//# sourceMappingURL=corpus_intelligence_input.js.map