...
exports.INTERPRETATION_PROFILE_HASH = exports.EXTRACTOR_VERSIONS = exports.INTERPRETATION_PROFILE_VERSION = exports.INTERPRETATION_PROFILE_ID = void 0;
exports.interpretationProfile = interpretationProfile;
...
exports.INTERPRETATION_PROFILE_HASH = `sha256:${stableDigest({
    id: exports.INTERPRETATION_PROFILE_ID,
    version: exports.INTERPRETATION_PROFILE_VERSION,
    extractor_versions: exports.EXTRACTOR_VERSIONS,
...
function interpretationProfile() {
    return {
        id: exports.INTERPRETATION_PROFILE_ID,
        version: exports.INTERPRETATION_PROFILE_VERSION,
        hash: exports.INTERPRETATION_PROFILE_HASH,
        extractorVersions: exports.EXTRACTOR_VERSIONS,
    };
}
function tomlScalarText(raw) {
...
function parseTomlSubset(text) {
    const doc = { values: new Map(), lines: new Map(), tables: new Set(), unreadable: [] };
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (!assignment) {
            doc.unreadable.push(`line ${index + 1}`);
            continue;
        }
        ...
        if (rest.startsWith("[") && !rest.endsWith("]")) {
            ...
            if (!closed) {
                doc.unreadable.push(`unterminated array at line ${index + 1}`);
                continue;
            }
            rest = collected.join(" ");
        }
        ...
        if (rest.startsWith("[") && rest.endsWith("]")) {
            ...
            if (items.some((item) => item === null)) {
                doc.unreadable.push(qualified);
                continue;
            }
            doc.values.set(qualified, items);
            continue;
        }
        if (rest.startsWith("{") && rest.endsWith("}")) {
...
function tomlChildKeys(doc, table) {
    const prefix = `${table}.`;
    const keys = new Set();
...
function emit(context, extractorId, kind, evidenceClass, value, detail, lineNumber) {
    const draft = {
        kind,
        extractorId,
        extractorVersion: exports.EXTRACTOR_VERSIONS[extractorId] ?? "0.0.0",
        evidenceClass,
        value,
        detail,
        sourceRef: {
...
function note(context, extractorId, code, message) {
...
function extractPyProject(context) {
    ...
    if (resolved.length === 0) {
        note(context, PACKAGE_MANIFEST, "package-manager-undetermined", "pyproject.toml declares no tool table or build backend this extractor recognizes; the package manager stays unknown");
    }
    for (const [manager, , rule] of resolved) {
        emit(context, PACKAGE_MANIFEST, "package_manager", "declared", manager, { rule }, doc.lines.get("build-system.build-backend"));
    }
    const identities = [
    ...
    if (typeof pythonConstraint === "string" && pythonConstraint !== "") {
        emit(context, PACKAGE_MANIFEST, "runtime_constraint", "declared", `python ${pythonConstraint}`, { runtime: "python", constraint: pythonConstraint }, doc.lines.get("tool.poetry.dependencies.python") ?? doc.lines.get("project.requires-python"));
    }
    const dependencies = new Map();
    for (const key of tomlChildKeys(doc, "tool.poetry.dependencies")) {
        if (key === "python")
            continue; // the interpreter is a runtime constraint, not a dependency
    ...
    if (Array.isArray(declared)) {
        for (const requirement of declared) {
            ...
            if (!dependencies.has(name))
                dependencies.set(name, { scope: "runtime", ...(doc.lines.has("project.dependencies") ? { line: doc.lines.get("project.dependencies") } : {}) });
        }
    }
...
function extractPackageJson(context) {
    let parsed;
    try {
        parsed = JSON.parse(context.text);
    }
    catch (error) {
    ...
    if (declaredManager !== "") {
        ...
        note(context, PACKAGE_MANIFEST, "package-manager-undetermined", "package.json declares no packageManager field; the installing client is not determined by this file");
    }
    ...
    for (const [field, scope] of [["dependencies", "runtime"], ["devDependencies", "development"]]) {
        const block = manifest[field];
        if (typeof block !== "object" || block === null || Array.isArray(block))
            continue;
        for (const name of Object.keys(block).sort(compareCodePoints)) {
            emit(context, PACKAGE_MANIFEST, "declared_dependency", "declared", name, { ecosystem: "npm", scope });
        }
    }
}
function extractCargoToml(context) {
    const doc = parseTomlSubset(context.text);
 ...
 */
function extractServiceSpec(context) {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (!/^\s/.test(raw)) {
            ...
            section = top && (top[1] === "service" || top[1] === "actions") ? top[1] : null;
            continue;
        }
        if (section === null)
            continue;
        ...
        if (item) {
            ...
            if (item[1] === "name")
                pendingAction.name = yamlScalarText(item[2]);
            else if (item[1] === "description")
                pendingAction.description = yamlScalarText(item[2]);
            continue;
        }
        const child = /^\s{4}([A-Za-z_][\w-]*):\s*(.*)$/.exec(raw);
        if (child && pendingAction) {
            if (child[1] === "name")
                pendingAction.name = yamlScalarText(child[2]);
            else if (child[1] === "description")
                pendingAction.description = yamlScalarText(child[2]);
        }
    }
    flush();
    ...
    if (actions.length === 0) {
        note(context, SERVICE_SPEC, "service-spec-no-actions", "no top-level `actions:` sequence of `- name:` entries was read from this spec");
    }
}
 ...
 */
function extractPythonRoutes(context) {
    const lines = context.text.split(/\r?\n/);
    for (let index = 0; index < lines.length; index++) {
        ...
        if (!definition) {
            note(context, PYTHON_ROUTES, "route-handler-unresolved", `a ${method} ${routePath} decorator has no handler definition directly beneath it; the handler symbol stays unknown`);
            continue;
        }
...
const BY_FILE_NAME = Object.freeze({
...
function extractorFor(record) {
    const byName = BY_FILE_NAME[record.file_name];
    if (byName)
        return byName;
    if ((record.extension ?? "").toLowerCase() === ".py")
...
function interpretRepository(input) {
    ...
    facts.sort((a, b) => compareCodePoints(a.kind, b.kind)
...
//# sourceMappingURL=repository_interpretation.js.map