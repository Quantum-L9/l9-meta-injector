...
exports.buildCorpusIntelligenceBundle = buildCorpusIntelligenceBundle;
exports.writeCorpusIntelligenceBundle = writeCorpusIntelligenceBundle;
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
const ordering_1 = require("./ordering");
const repository_model_1 = require("./repository_model");
exports.CORPUS_INTELLIGENCE_PACKET_TYPE = "l9.corpus-intelligence";
...
function corpusPayloadPath(field) {
...
class CorpusIntelligenceError extends Error {
}
exports.CorpusIntelligenceError = CorpusIntelligenceError;
/**
 ...
 */
const SCHEMA_HASH = (0, repository_model_1.semanticHash)({
...
function packetRef(root) {
    ...
    if (packet.artifact_hash !== undefined)
        reference.artifact_hash = packet.artifact_hash;
    return reference;
}
/**
 ...
 */
function artifactIdentities(roots) {
 ...
 * it is still in hand.
 */
function validateCorpusIntelligencePacket(packet, payload, roots) {
    const errors = [];
    ...
    for (const reference of packet.inputs.repository_model_packets) {
        if (reference.packet_type !== "l9.repository-model") {
            errors.push(`input ${reference.packet_id} is a ${reference.packet_type}, not an l9.repository-model packet`);
        }
    }
    for (const root of packet.corpus.root_refs) {
        if (!declared.has(root.repository_model_packet.packet_id)) {
            errors.push(`root ${root.root_id} references packet ${root.repository_model_packet.packet_id}, which is not a declared input`);
        }
    }
    ...
    const resolve = (identity, context) => {
    ...
    for (const signal of payload.document_work_signals) {
        ...
        if (kind === "line" && !LINE_BEARING_FORMATS.has(signal.document_format)) {
            errors.push(`work signal ${signal.signal_id} claims a line locator for format ` +
    ...
    for (const relation of payload.semantic_pair_relations) {
        ...
        if (!CONFIDENCE_CLASSES.has(relation.confidence_class)) {
            errors.push(`pair relation ${relation.relation_id} declares confidence class '${relation.confidence_class}'`);
        }
    }
    ...
    for (const [field, expected] of candidateDomains) {
        for (const cluster of payload[field]) {
            knownCandidates.add(cluster.candidate_id);
            if (cluster.candidate_type !== expected) {
                errors.push(`${field} carries ${cluster.candidate_id} declared as ` +
    ...
    for (const request of payload.reasoning_candidates) {
        if (!knownCandidates.has(request.candidate_id)) {
            errors.push(`reasoning candidate ${request.reasoning_candidate_id} names candidate ` +
                `${request.candidate_id}, which this packet does not carry`);
        }
        if (!UPSTREAM_REASONING_TYPES.has(request.recommended_reasoning_type)) {
            errors.push(`reasoning candidate ${request.reasoning_candidate_id} recommends ` +
    ...
    for (const record of payload.readiness_evidence) {
        if (!knownCandidates.has(record.subject_id) && !identities.has(record.subject_id)) {
            errors.push(`readiness ${record.readiness_id} is about ${record.subject_id}, which is ` +
...
function buildCorpusIntelligencePacket(input) {
    ...
    if (input.roots.length === 0) {
        throw new CorpusIntelligenceError("a corpus intelligence packet is an analysis over repository model packets; " +
    ...
    const inputs = {
        repository_model_packets: input.roots
            .map(packetRef)
            .sort((a, b) => (0, ordering_1.compareCodePoints)(a.packet_id, b.packet_id)),
    };
    const rootRefs = input.roots
        .map((root) => {
        const reference = {
            root_id: root.rootId,
        ...
        if (root.repositoryId !== undefined)
            reference.repository_id = root.repositoryId;
        return reference;
    })
    ...
    const shell = {
        ...
        packet_version: exports.CORPUS_INTELLIGENCE_PACKET_VERSION,
        packet_id: "",
        producer: { name: exports.CORPUS_INTELLIGENCE_PRODUCER_NAME, version: input.producerVersion },
    ...
    const semantic = (0, repository_model_1.semanticHash)({
 ...
 * the other.
 */
function buildCorpusIntelligenceBundle(packet, payload, options) {
    const files = [];
    const entries = [];
    ...
    entries.sort((a, b) => (0, ordering_1.compareCodePoints)(a.path, b.path));
    const manifest = {
        manifest_version: exports.CORPUS_INTELLIGENCE_MANIFEST_VERSION,
        packet_id: packet.packet_id,
        packet_type: packet.packet_type,
        packet_version: packet.packet_version,
        semantic_hash: packet.semantic_hash,
        // Recomputed by the consumer over the file entries alone.
        artifact_hash: (0, repository_model_1.semanticHash)(entries),
        files: entries,
        created_at: options.createdAt,
    };
    const { contents } = canonicalDocument(manifest);
    files.push({ path: MANIFEST_RELATIVE_PATH, contents });
    files.sort((a, b) => (0, ordering_1.compareCodePoints)(a.path, b.path));
    return { files, manifest };
}
/** Write a bundle to disk. Refuses to write into a non-empty directory. */
function writeCorpusIntelligenceBundle(bundle, outDir) {
    const bundleRoot = path.resolve(outDir);
    if (fs.existsSync(bundleRoot) && fs.readdirSync(bundleRoot).length > 0) {
        throw new CorpusIntelligenceError(`corpus-intelligence: packet bundles are immutable; refusing to write into a non-empty directory: ${bundleRoot}`);
    }
...
//# sourceMappingURL=corpus_intelligence.js.map