...
function statusRows(index, statuses) {
...
function renderCorpusReport(index) {
    ...
    lines.push("Read-only observation. No file under the source was moved, deleted, rewritten, or consolidated.", "");
    ...
    lines.push(...table(["measure", "value"], [
        ...
        ["interpretation profile", code(index.repository_model.interpretation_profile
    ...
    lines.push("Every row below is an explicit statement a document makes about itself, cited to the", "line that makes it. Nothing here is inferred from a filename, a path, a modification", "time, or the absence of a signal. Where a document states two conflicting things, both", "appear.", "");
    lines.push("### Plans and Roadmaps", "");
    lines.push(...table(["artifact", "declared kind", "declared title", "declared status"], planRows(index), "No artifact declares a work kind."));
    lines.push("### WIP and Drafts", "");
    lines.push(...table(["artifact", "declared status", "declared kind", "open tasks"], statusRows(index, ["wip", "draft"]), "No artifact declares itself WIP or draft."));
    lines.push("### Blocked Work", "");
    lines.push(...table(["artifact", "declared status", "declared kind", "open tasks"], statusRows(index, ["blocked"]), "No artifact declares itself blocked."));
    lines.push("### Open Tasks", "");
    lines.push(...table(["artifact", "line", "task", "confidence"], signalRows(byPredicate(index, "work.task.open")), "No open task is declared."));
    lines.push("### Completed Tasks", "");
    lines.push(...table(["artifact", "line", "task", "confidence"], signalRows(byPredicate(index, "work.task.completed")), "No completed task is declared."));
    lines.push("### Milestones", "");
    lines.push(...table(["artifact", "line", "milestone", "confidence"], signalRows(byPredicate(index, "work.milestone")), "No milestone is declared."));
    lines.push("## Explicit Relationships", "");
    lines.push("Declared pointers, carried as the exact target text each document wrote. A target is", "not resolved to an artifact unless the document named an observed path outright.", "");
    ...
    lines.push("Byte-identical files. Membership is content-hash equality, so a physical file and a", "file inside an archive can share a cluster. The representative is a deterministic", "rendering anchor — every member of a cluster is exactly equivalent to every other, and", "naming one says nothing about which copy anything should be done with.", "");
    lines.push(...table(["count", "recoverable bytes", "representative", "other members"], index.exact_duplicate_clusters.map((cluster) => [
        `${cluster.count}`,
    ...
    lines.push(`Candidates from \`${profile.near_duplicate_method}\` at a lexical similarity threshold`, `of ${profile.near_duplicate_threshold}. The score is the exact Jaccard overlap of the unique`, "5-token shingles of two normalized documents.", "", "A candidate means the two documents share wording. It does not establish that they", "cover one subject, belong to one effort, supersede one another, or that anything", "should be done about them. Byte-identical files are excluded here and reported as", "exact duplicates above.", "");
    lines.push(...table(["score", "artifact a", "artifact b", "shared/union shingles"], candidateRows(index.near_duplicate_candidates), profile.near_duplicate_enabled
    ...
    lines.push("Archives are observed in place and expanded into tool-owned staging. Members are", "carried as virtual artifacts named `<archive>!/<member>`; nothing is extracted beside", "the source.", "");
    lines.push(...table(["archive", "depth", "expanded", "members", "omitted", "holds"], index.archives.map((archive) => [
    ...
    lines.push("What the observation could not establish, carried forward rather than rounded away.", "");
    lines.push("### Packet diagnostics", "");
    lines.push(...table(["code", "severity", "count"], index.diagnostics.packet.map((entry) => [code(entry.code), entry.severity, `${entry.count}`]), "The packet reported no diagnostics."));
    lines.push("### Interpretation diagnostics", "");
    lines.push(...table(["code", "severity", "count"], index.diagnostics.interpretation.map((entry) => [code(entry.code), entry.severity, `${entry.count}`]), "Interpretation reported no diagnostics."));
    lines.push("### Artifacts outside the similarity analysis", "");
    lines.push(...table(["reason", "count"], [...index.diagnostics.near_duplicate_excluded]
        .sort((left, right) => (0, ordering_1.compareCodePoints)(left.reason, right.reason))
        .map((entry) => [code(entry.reason), `${entry.count}`]), "Every observed artifact was eligible for the similarity analysis."));
    return `${lines.join("\n").replace(/\n{3,}/g, "\n\n").trimEnd()}\n`;
}
